...
package Fish::3rdParty::MozCache;

BEGIN {
    @ISA = 'Exporter';
    @EXPORT = qw/ cache_read read_map get_cache_map recover /;
}
...
sub recover
{
my ($entry_ref, $recover_dir, $pref) = @_;
...
if (length $data == 0)
{
        say "Tried to recover empty entry, skipping.";
	return;
}
...
my $md = "$recover_dir/.metadata";
-d $md or `mkdir -p "$md"`;
my $filename2 = "$md/$filename";

$filename = $recover_dir . "/" . $pref . $filename;
